...
export function getProjection<T>(
    ...
    let parent = findParentWhichCanHaveChildren(directParent, activeItem, canRootHaveChildren);
    if (parent === undefined) return null;
    ...
    function findParentWhichCanHaveChildren(
        ...
        if (!parent) {
            const rootCanHaveChildren =
                typeof canRootHaveChildren === 'function' ? canRootHaveChildren(dragItem) : canRootHaveChildren;
            if (rootCanHaveChildren === false) return undefined;
            return parent;
        }
        const canHaveChildren =
            typeof parent.canHaveChildren === 'function' ? parent.canHaveChildren(dragItem) : parent.canHaveChildren;
...
export function getIsOverParent<T>(parent: FlattenedItem<T> | null, overId: UniqueIdentifier): boolean {
